...
function getNextSectionChapterId($currentChapterSEQNo, $section_act_type) {
//    echo $currentChapterSEQNo;
    $NextChapterQuery = "select id from coi_sections_chapter where chapter_seq_no = (select min(chapter_seq_no) from coi_sections_chapter where chapter_seq_no > $currentChapterSEQNo) and section_act_type = '".$section_act_type."'";
    $NextArticleIdResult2 = mysqli_query($GLOBALS['con'],$NextChapterQuery);
    $NextArticleIdRow2 = mysqli_fetch_array($NextArticleIdResult2);
    $nextArticleId2 = $NextArticleIdRow2[0];
    //check if articles exists in the chapter or not.
    if ($nextArticleId2) {
        $NextChapterQuery = "select count(*) from coi_sections where chapter_id = $nextArticleId2";
        $NextArticleIdResult3 = mysqli_query($GLOBALS['con'],$NextChapterQuery);
        $NextArticleIdRow3 = mysqli_fetch_array($NextArticleIdResult3);
        $total_count = $NextArticleIdRow3[0];
        if ($total_count == 0) {
            $nextArticleId2 = getNextSectionChapterId($NextArticleIdRow2[1]);
        }
    }
...
function getPrevSectionChapterId($currentChapterSEQNo, $section_act_type) {
    $PrevChapterQuery = "select id from coi_sections_chapter where chapter_seq_no = (select max(chapter_seq_no) from coi_sections_chapter where chapter_seq_no < $currentChapterSEQNo) and section_act_type = '".$section_act_type."'";
    $PrevArticleIdResult2 = mysqli_query($GLOBALS['con'],$PrevChapterQuery);
    $PrevArticleIdRow2 = mysqli_fetch_array($PrevArticleIdResult2);
    $prevArticleId2 = $PrevArticleIdRow2[0];
    //checking if article exists or not
    if ($prevArticleId2) {
        $prevChapterQuery = "select count(*) from coi_sections where chapter_id = $prevArticleId2";
        $prevArticleIdResult3 = mysqli_query($GLOBALS['con'],$prevChapterQuery);
        $prevArticleIdRow3 = mysqli_fetch_array($prevArticleIdResult3);
        $total_count = $prevArticleIdRow3[0];
        if ($total_count == 0) {
            $prevArticleId2 = getPrevSectionChapterId($PrevArticleIdRow2[1]);
        }
    }
...
function getNextSectionURL($c_section_seq_no, $c_chapter_id, $c_chapter_seq_no, $section_type, $section_act_type) {
//    echo "$c_section_seq_no, $c_chapter_id, $c_chapter_seq_no";die;
    $nextQuery = "select id from coi_sections where section_seq_no = (select min(section_seq_no) from coi_sections where section_seq_no > '$c_section_seq_no' and chapter_id=$c_chapter_id) and chapter_id=$c_chapter_id";

    $NextSectionIdResult = mysqli_query($GLOBALS['con'],$nextQuery);
    $NextSectionIdRow = mysqli_fetch_array($NextSectionIdResult);
    $nextSectionId = $NextSectionIdRow[0];

    if (!$nextSectionId) {
        $nextChapterId = getNextSectionChapterId($c_chapter_seq_no, $section_act_type);
        $nextQuery2 = "select id from coi_sections where section_seq_no = (select min(section_seq_no) from coi_sections where chapter_id=($nextChapterId)) and chapter_id = $nextChapterId";

        $NextSectionIdResult2 = mysqli_query($GLOBALS['con'],$nextQuery2);
        $NextSectionIdRow2 = mysqli_fetch_array($NextSectionIdResult2);
        $nextSectionId = $NextSectionIdRow2[0];
    }

    $NextPageUrl = '';

    if ($nextSectionId) {
        $encryptyNextId = base64_encode(base64_encode($nextSectionId));
        $NextPageUrl = 'showiframe3?V1Zaa1VsQlJQVDA9=' . $encryptyNextId . '&showLOAsections';
    }
    return $NextPageUrl;
}

function getPrevSectionURL($c_section_seq_no, $c_chapter_id, $c_chapter_seq_no, $section_type, $section_act_type) {
    $prevQuery = "select id from coi_sections where section_seq_no = (select max(section_seq_no) from coi_sections where section_seq_no < '$c_section_seq_no' and chapter_id = $c_chapter_id) and chapter_id = $c_chapter_id";
//    echo $prevQuery;
    $prevSectionIdResult = mysqli_query($GLOBALS['con'],$prevQuery);
    $prevSectionIdRow = mysqli_fetch_array($prevSectionIdResult);
    $prevSectionId = $prevSectionIdRow[0];
    if (!$prevSectionId) {

        //try for prev chapter
        //get prev chatper with ChapterSEQ No:
        $prevChapterId = getPrevSectionChapterId($c_chapter_seq_no, $section_act_type);
        if ($prevChapterId) {
            $prevQuery2 = "select id from coi_sections where section_seq_no = (select max(section_seq_no) from coi_sections where chapter_id = $prevChapterId) and chapter_id = $prevChapterId";
            $prevSectionIdResult2 = mysqli_query($GLOBALS['con'],$prevQuery2);
            $prevSectionIdRow2 = mysqli_fetch_array($prevSectionIdResult2);
            $prevSectionId = $prevSectionIdRow2[0];
        }
    }
    $PrevPageUrl = '';
    if ($prevSectionId) {
        $encryptyPrevId = base64_encode(base64_encode($prevSectionId));
        $PrevPageUrl = 'showiframe3?V1Zaa1VsQlJQVDA9=' . $encryptyPrevId . '&showLOAsections';
    }
    return $PrevPageUrl;
}


function getNextGSTCouncilMeetingChapterId($currentChapterNo) {
//    echo $currentChapterNo;
    //$NextChapterQuery = "select id, chapter_no from gst_council_meeting_chapter where chapter_no = (select min(chapter_no) from gst_council_meeting_chapter where chapter_no > $currentChapterNo)";
    $NextChapterQuery = "select id,chapter_no from gst_council_meeting_chapter where chapter_no > $currentChapterNo";
    $NextGSTCouncilMeetingIdResult2 = mysqli_query($GLOBALS['con'],$NextChapterQuery);
    $NextGSTCouncilMeetingIdRow2 = mysqli_fetch_array($NextGSTCouncilMeetingIdResult2);
    $nextGSTCouncilMeetingId2 = $NextGSTCouncilMeetingIdRow2[0];
    //check if articles exists in the chapter or not.
    if ($nextGSTCouncilMeetingId2) {
        $nextChapterQuery = "select count(*) from gst_council_meeting where chapter_id = $nextGSTCouncilMeetingId2";
        $nextGSTCouncilMeetingIdResult3 = mysqli_query($GLOBALS['con'],$nextChapterQuery);
        $nextGSTCouncilMeetingIdRow3 = mysqli_fetch_array($nextGSTCouncilMeetingIdResult3);
        $total_count = $nextGSTCouncilMeetingIdRow3[0];
        if ($total_count == 0) {
             $nextGSTCouncilMeetingId2 = getNextGSTCouncilMeetingChapterId($NextGSTCouncilMeetingIdRow2[1]);
        }
    }

    return $nextGSTCouncilMeetingId2;
}

function getPrevGSTCouncilMeetingChapterId($currentChapterNo) {
    //echo $currentChapterNo;
    //$PrevChapterQuery = "select id, chapter_no from gst_council_meeting_chapter where chapter_no = (select max(chapter_no) from gst_council_meeting_chapter where chapter_no < $currentChapterNo)";
    $PrevChapterQuery = "select id,chapter_no from gst_council_meeting_chapter where chapter_no < $currentChapterNo order by id DESC";
    $PrevGSTCouncilMeetingIdResult2 = mysqli_query($GLOBALS['con'],$PrevChapterQuery);
    $PrevGSTCouncilMeetingIdRow2 = mysqli_fetch_array($PrevGSTCouncilMeetingIdResult2);
    $prevGSTCouncilMeetingId2 = $PrevGSTCouncilMeetingIdRow2[0];
    //checking if article exists or not
    if ($prevGSTCouncilMeetingId2) {
        $prevChapterQuery = "select count(*) from gst_council_meeting where chapter_id = $prevGSTCouncilMeetingId2";
        $prevGSTCouncilMeetingIdResult3 = mysqli_query($GLOBALS['con'],$prevChapterQuery);
        $prevGSTCouncilMeetingIdRow3 = mysqli_fetch_array($prevGSTCouncilMeetingIdResult3);
        $total_count = $prevGSTCouncilMeetingIdRow3[0];
        if ($total_count == 0) {
            $prevGSTCouncilMeetingId2 = getPrevGSTCouncilMeetingChapterId($PrevGSTCouncilMeetingIdRow2[1]);
        }
    }

    return $prevGSTCouncilMeetingId2;
}

function getNextGSTCouncilMeetingURL($meeting_number, $chapter_id, $chapter_no) {
    $nextQuery = "select id from gst_council_meeting where meeting_no = (select min(meeting_no) from gst_council_meeting where meeting_no > '$meeting_number' and chapter_id = $chapter_id) and chapter_id = $chapter_id";

    $NextGSTCouncilMeetingIdResult = mysqli_query($GLOBALS['con'],$nextQuery);
    $NextGSTCouncilMeetingIdRow = mysqli_fetch_array($NextGSTCouncilMeetingIdResult);
    $nextGSTCouncilMeetingId = $NextGSTCouncilMeetingIdRow[0];

    if (!$nextGSTCouncilMeetingId) {
        $nextChapterId = getNextGSTCouncilMeetingChapterId($chapter_no);
        $nextQuery2 = "select id from gst_council_meeting where meeting_no = (select min(meeting_no) from gst_council_meeting where chapter_id = $nextChapterId) and chapter_id = $nextChapterId";

        $NextGSTCouncilMeetingIdResult2 = mysqli_query($GLOBALS['con'],$nextQuery2);
        $NextGSTCouncilMeetingIdRow2 = mysqli_fetch_array($NextGSTCouncilMeetingIdResult2);
        $nextGSTCouncilMeetingId = $NextGSTCouncilMeetingIdRow2[0];
    }

    $NextPageUrl = '';

    if ($nextGSTCouncilMeetingId) {
        $encryptyNextId = base64_encode(base64_encode($nextGSTCouncilMeetingId));
        $NextPageUrl = 'showiframe4?V1Zaa1VsQlJQVDA9=' . $encryptyNextId . '&showGSTCouncilMeeting';
    }
    return $NextPageUrl;
}

function getPrevGSTCouncilMeetingURL($meeting_number, $chapter_id, $chapter_no) {
    $prevQuery = "select id from gst_council_meeting where meeting_no = (select max(meeting_no) from gst_council_meeting where meeting_no < '$meeting_number' and chapter_id = $chapter_id) and chapter_id = $chapter_id";
//    echo $prevQuery;
    $prevGSTCouncilMeetingIdResult = mysqli_query($GLOBALS['con'],$prevQuery);
    $prevGSTCouncilMeetingIdRow = mysqli_fetch_array($prevGSTCouncilMeetingIdResult);
    $prevGSTCouncilMeetingId = $prevGSTCouncilMeetingIdRow[0];
    if (!$prevGSTCouncilMeetingId) {

        //try for prev chapter
        //get prev chatper with ChapterSEQ No:
        $prevChapterId = getPrevGSTCouncilMeetingChapterId($chapter_no);
        if ($prevChapterId) {
            $prevQuery2 = "select id from gst_council_meeting where meeting_no = (select max(meeting_no) from gst_council_meeting where chapter_id = $prevChapterId) and chapter_id = $prevChapterId";
            $prevGSTCouncilMeetingIdResult2 = mysqli_query($GLOBALS['con'],$prevQuery2);
            $prevGSTCouncilMeetingIdRow2 = mysqli_fetch_array($prevGSTCouncilMeetingIdResult2);
            $prevGSTCouncilMeetingId = $prevGSTCouncilMeetingIdRow2[0];
        }
    }
    $PrevPageUrl = '';
    if ($prevGSTCouncilMeetingId) {
        $encryptyPrevId = base64_encode(base64_encode($prevGSTCouncilMeetingId));
        $PrevPageUrl = 'showiframe4?V1Zaa1VsQlJQVDA9=' . $encryptyPrevId . '&showGSTCouncilMeeting';
    }
    return $PrevPageUrl;
}
?>
...
<?php if (isset($_GET['page']) && ($_GET['page'] == "video")) {
    ...
    if (isset($_GET['V1Zaa1VsQlJQVDA9'])) {
        ...
        } else if (isset($_GET['showLOAsections'])) {
            //get Section Table to be shown here.
            $qer = "SELECT cs.*,csc.chapter_no, csc.chapter_seq_no  FROM coi_sections cs LEFT OUTER JOIN coi_sections_chapter csc on csc.id=cs.chapter_id where cs.id = $decryptID";
            $result = mysqli_query($GLOBALS['con'],$qer);
        } else if (isset($_GET['showGSTCouncilMeeting'])) {
            //get GST Council Meeting Table to be shown here.
            $q = "SELECT cm.*,cmc.chapter_no,cmc.chapter_date FROM gst_council_meeting cm LEFT OUTER JOIN gst_council_meeting_chapter cmc on cmc.id=cm.chapter_id where cm.id = $decryptID";
            $result = mysqli_query($GLOBALS['con'],$q);
        } else {
...
//                print_r($row);
            ...
            if (isset($_GET['showLOAsections'])) {
                $file_path = $row['section_file'];
                $SectionNumber = $row['section_no'];
                $ChapterNo = $row['chapter_no'];
                $ChapterId = $row['chapter_id'];
                $ChapterSEQNo = $row['chapter_seq_no'];
                $c_section_seq_no = $row['section_seq_no'];
                $section_type = $row['section_type'];
                $section_act_type = $row['section_act_type'];

//                print_r($row);
                $destination_Path = '/data/Section/';

                $PrevPageUrl = getPrevSectionURL($c_section_seq_no, $ChapterId, $ChapterSEQNo, $section_type);
                $NextPageUrl = getNextSectionURL($c_section_seq_no, $ChapterId, $ChapterSEQNo, $section_type);
            } else {
                $file_path = $row['file_path'];
            }

            if (isset($_GET['showGSTCouncilMeeting'])) {
                $file_path = $row['meeting_file'];
                ...
                $PrevPageUrl = getPrevGSTCouncilMeetingURL($MeetingNumber, $ChapterId, $ChapterNo);
                $NextPageUrl = getNextGSTCouncilMeetingURL($MeetingNumber, $ChapterId, $ChapterNo);
            } else {
                $file_path = $row['file_path'];
            }
            ...
            <div class="col-md-16 col-sm-16 left-section">
                ...
                <form method="post" action="" id="formDownload" name="formDownload">
                    ...
                    <h1>
                        ...
                        if ((isset($_GET['page']) && $_GET['page'] == "recent") || (isset($_GET['datatable']) && $_GET['datatable'] != 'undefined')) {
                            ...
                            if ($section_type == 'Section') {
                                if ($section_act_type == 'LIMITATION'){
                                    echo "<span style='font-size: 16px; color: #666;'>THE LIMITATION ACT</span> |  Chapter $ChapterNo  - Section $SectionNumber";
                                }
                                if ($section_act_type == 'GENERAL'){
                                    echo "<span style='font-size: 16px; color: #666;'>THE GENERAL CLAUSES ACT</span> |  Chapter $ChapterNo  - Section $SectionNumber";
                                }
                                if ($section_act_type == 'INDIAN'){
                                    echo "<span style='font-size: 16px; color: #666;'>THE INDIAN CONTRACT ACT</span> |  Chapter $ChapterNo  - Section $SectionNumber";
                                }
                                if ($section_act_type == 'SALE'){
                                    echo "<span style='font-size: 16px; color: #666;'>THE SALE OF GOODS ACT</span> |  Chapter $ChapterNo  - Section $SectionNumber";
                                }
                                if ($section_act_type == 'TRANSFER'){
                                    echo "<span style='font-size: 16px; color: #666;'>THE TRANSFER OF PROPERTY ACT</span> |  Chapter $ChapterNo  - Section $SectionNumber";
                                }
                                
                            } else {
                                echo "<span style='font-size: 16px; color: #666;'>Limitation Of Acts</span> |  $section_type";
                            }
                        } else if (isset($_GET['showGSTCouncilMeeting'])) {
                                echo "<span style='font-size: 16px; color: #666;'>GST Council Meeting</span> |  <span style='text-transform: none;'>Meeting No. $ChapterNo  - $MeetingName ( $Chapter_Date )</span>";
                        }
                        ?>
                    ...
                    <div class="col-md-16">
                        ...
                        <?php
                        if (isset($_GET['page']) && ($_GET['page'] == "articles" || $_GET['page'] == "taxvista" || $_GET['page'] == "highlights" || $_GET['page'] == "features" || $_GET['page'] == "budgets_analysis" || $_GET['page'] == "unionBudget" || $_GET['page'] == "stateBudget" || $_GET['page'] == "showIframe")) {
                            ?>
                            ...
                            if (isset($_GET['page']) && ($_GET['page'] == "recent")) {
                                ...
                                $tableName = 'coi_sections';
                                $rowProduct['ProductName'] = 'LOA';
                            }
                            ...
                            if (($_SESSION['vatAccess'] == 'Y') || ($_SESSION['STAccess'] == 'Y') || ($_SESSION['CEAccess'] == 'Y') || ($_SESSION['customsAccess'] == 'Y') || ($_SESSION['gstAccess'] == 'Y')) {
                                ...
                                if (($rowProduct['ProductName'] == "VAT" || $rowProduct['ProductName'] == "COI" || $rowProduct['ProductName'] == "LOA" || $rowProduct['ProductName'] == "SGST" || $rowProduct['ProductName'] == "UTGST") && ($_SESSION['vatAccess'] == 'Y')) {
                                ...
                                if ($_SESSION["userStatus"] == "expired") {
                                    ...
                                    <div class="pull-right text-right b-margin-10">
                                        ...
                                        <ul class="list-inline" >
                                            ...
                                            <li>
                                                <input type="hidden" value="<?php echo encrypt_url($destination_Path . $file_path); ?>" name="file_path" id="file_path" />
...
//                                    echo $rowProduct['ProductName'];die;
                                    if (isset($rowProduct['ProductName']) && $rowProduct['ProductName'] != '' && $rowProduct['ProductName'] != 'COI' && $rowProduct['ProductName'] != 'LOA') {
                                            ...
                                            <div class='cited-in-block'>
                                                ...
                                                if ($citedin_count > 0) {
                                                    ...
                                                    if (count($citeIn['Circular']) > 0) {
                                                        ?>
                                                        <div class="expanding-blocks" data-block='citedin-circular'>Circular /Order/ROD (<?php echo count($citeIn['Circular']); ?>)<span class="expand">+</span></div>
                                                        <?php 
                                                    ...
                                                    if (count($citeIn['Section']) > 0) {
                                                        ?>
                                                        <div class="expanding-blocks" data-block='citedin-section'>Sections (<?php echo count($citeIn['Section']); ?>)<span class="expand">+</span></div>
                                                        <?php
                                    ...
                                    <div id='rightDisplayBordered' class="bordered" style="width:<?php echo $bordered_width;?>;float:right">
                                        ...
                                        if ($file_extn == 'pdf') {
                                            ?> src='<?php echo $getBaseUrl . $destination_Path . $file_path; ?>' <?php
                                                    ...
                                                    ?> src='<?php echo "-?l=" . encrypt_url($destination_Path . $file_path); ?>' <?php
                                                }
                                                ?> frameborder='0' allowtransparency='true' scrolling='no' width="100%" <?php
                                                if ($file_extn == 'pdf') {
                                                    echo " height='1130' ";
