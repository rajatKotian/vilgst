<?php
$dataType = 'EditMappingCaseNotification';
$pageType = 'EditMappingCaseNotification';
$pageHead = '';

include('header.php');
$addText = ' - Mapping Case Notification';

if (isset($_GET['id'])) {
    //get Mapping Case Cite detail by id
    $id = cleanForDB($_GET['id']);
    $Mapping_Case_Notification = getMappingCaseNotificationDatabyId($_GET['id']);
}
$Mapping_Case_Notification_id = 0;
if (count($Mapping_Case_Notification) > 0 && $Mapping_Case_Notification['id'] == $id) {
    $validId = true;
    $Mapping_Case_Notification_id = $id;
} else {
    $validId = false;
}
?>
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <input type="hidden" id="Mapping_Case_Notification_id" value="<?php echo "$Mapping_Case_Notification_id"; ?>" />
...
<script type="text/javascript">
    $(document).ready(function() {
        if ($('#Mapping_Case_Notification_id').val() != 0) {
            loadEditFormCOI('mapping_case_notification', $('#Mapping_Case_Notification_id').val());
        }else{
            $('.alert-error').show();
        }
    });
