<?php
$dataType = '';
$pageType = 'COIArticleListArticles';
$pageHead = '';
ini_set('display_errors', 0);
include('header.php');
$addText = 'All COI Articles';
$chapter_id = 0;
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="box-tools pull-right" style="margin-left:10px;">
                                <a href="COI_ChapterAdd.php">Add Part</a>           
                            </div>
                            <div class="box-tools pull-right">
                                <a href="COI_ArticleAdd.php">Add Article</a>           
                            </div>
                        </div>
                    </div>
                        ...
                        <table id="dataResult" class="display table table-bordered table-hover table-blue" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>Article No. (Display)</th>
                                    <th style="min-width: 150px;">Article Name</th>
                                    <th>Part No</th>
                                    <th style="min-width: 50px;">Article File</th>
                            ...
                            <tbody>
                                ...
                                $articles = listCOIArticles($chapter_id);
                                if (count($articles) > 0) {
                                    foreach ($articles as $article) {
                                        ?>
                                        <tr>
                                            <td><p><?php echo $article['id']; ?></p></td>
                                            <td><p><?php echo $article['article_seq_no']; ?></p></td>
                                            <td><p><?php echo $article['article_type'] . " " . $article['article_no']; ?></p></td>
                                            <td><p><?php echo $article['article_name']; ?></p></td>
                                            <td><p><?php
                                                    if ($article['article_type'] == 'Article') {
                                                        echo "Part " . $article['chapter_no'];
                                                        $ArticleNameDisplay = " Article " . $article['article_no'];
                                                    } else {
                                                        echo $article['chapter_no'];
                                                        $ArticleNameDisplay =  $article['chapter_no'];
                                                    }
                                                    ?></p></td>
                                            <td><p>
                                                    <?php
                                                    $encryptyId = base64_encode(base64_encode($article['id']));

                                                    echo "<a  href='../showiframe?V1Zaa1VsQlJQVDA9=" . $encryptyId . "&showCOIarticles' target='_blank' >" . $ArticleNameDisplay . "</a><button class='copy-file-link badge bg-navy'>Copy</button>";
                                                    ?>
                                                </p>
                                            </td>
                                            <!--<td><p><?php echo $article['updated_on']; ?></p></td>-->
                                            <td>
                                                <p>

                                                    <a href="COI_editHTMLArticleFile.php?V1Zaa1VsQlJQVDA9=<?php echo $encryptyId; ?>&table=coi_articles&primarykey=id&name=<?php echo $article['article_no']; ?>&title=<?php echo $article['article_name']; ?>" data-id="<?php echo $encryptyId ?>" data-table="coi_articles" data-name="<?php echo $article['article_name']; ?>" data-title="COI Article" data-toggle="tooltip" class="btn btn-warning edit-html" title="" data-original-title="Edit HTML" target="_blank">
                                                        <i class="fa fa-file-code-o"></i></a>

                                                    <a href='COI_ArticleEdit.php?id=<?php echo $article['id']; ?>' data-id='<?php echo $article['id']; ?>' data-table='coi_article' data-toggle='tooltip' 
                                                       class='btn btn-success update-data' title='Edit Data'><i class='fa fa-edit'></i>
                                                    </a>  
                                                    <a href='#' data-id='<?php echo $article['id']; ?>' data-table='coi_article' data-toggle='tooltip' class='btn btn-danger delete-data' 
...
//        viewDataTableCustom();
        $('#dataResult').dataTable({
            ...
            "drawCallback": function() {
                loadPopupModalDeleteAll('coi_article', 'delete');
                copyLinkToClipboard();
//                initHtmlEditPage();
            },
