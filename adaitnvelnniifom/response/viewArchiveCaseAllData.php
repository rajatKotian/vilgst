...
$columns = array(
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
                $getDbRecord = getDbRecord('product', 'prod_id', $d);
                $prodType = $getDbRecord[0]['dbsuffix']; 
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
                $getDbRecord = getDbRecord('product', 'prod_id', $d);
                $prodtitle = $getDbRecord[0]['prod_name']; 
...
} else if (isset($_GET['prod_id']) || isset($_GET['party_name'])) {
    $prod_id = isset($_GET['prod_id']) ? $_GET['prod_id'] : '0';
    $party_name = isset($_GET['party_name']) ? $_GET['party_name'] : '';
    if($prod_id != '' && $prod_id != '0' && $party_name != '') {
        $whereAll = " prod_id='".$prod_id."' AND party_name like '%".$party_name."%'";
    } else if($prod_id != ''  && $prod_id != '0') {
        $whereAll = " prod_id='".$prod_id."'";
    } else if($party_name != '') {
...
require( 'scripts/ssp.class.all.php' );
