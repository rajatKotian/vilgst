<?php
	...
	// $file_extn = strtolower(substr($filename,-3));
		...
		$result = mysqli_query($GLOBALS['con'],"SELECT file_data,circular_no FROM $table where  data_id = '$id'") or die(mysqli_error());
		$row = mysqli_fetch_array($result);

		$query = mysqli_query($GLOBALS['con'],"SELECT dbsuffix FROM product where  prod_id = '$prod_id'") or die(mysqli_error());
		$getdata = mysqli_fetch_array($query, MYSQL_ASSOC);
		
		$filename=$row['circular_no'].'-'. $getdata['dbsuffix'].'.htm';

		

		//$root_path = $_SERVER['DOCUMENT_ROOT']."/harry/Vligst Download/viewdata.htm"; 
		 

		
		$data=stripslashes($row['file_data']); 
	...
	// if($file_extn != 'pdf') {
		...
		$content = "<div style='font-family:Verdana; font-size:8.5pt; padding:5px; border:1px solid #c6bdac; margin:10px; background:#faf8f4; color:#422b03'>Downloaded from www.vatinfoline.com on $curDate </div>".$data;
...
<?php 
			...
			$decryptID = base64_decode(base64_decode($_GET['V1Zaa1VsQlJQVDA9']));
				...
				if(isset($_GET['page']) && ($_GET['page']=="recent")) {
				   ...
				   $result = mysqli_query($GLOBALS['con'],"SELECT circular_no, file_path, cir_subject 'summary', sub_prod_id, prod_id,file_data FROM recent_data where  data_id = '$decryptID'");
				   ...
				   $result = mysqli_query($GLOBALS['con'],"SELECT subject  'circular_no', file_path, summary, analysis_id 'sub_prod_id'  FROM budgets_analysis where  analysis_id = '$decryptID'");
					...
					$result = mysqli_query($GLOBALS['con'],"SELECT circular_no, file_path, cir_subject 'summary', sub_prod_id, prod_id,file_data FROM $tableName where  data_id = '$decryptID'");
    ...
    <form method="post" action="" id="formDownload" name="formDownload">
    	<input type="hidden" name="id" value="<?php echo $decryptID?>">
    	...
    	<input type="hidden" name="prod_id" value="<?php echo $row['prod_id']?>">
			      ...
			      if(isset($_GET['page']) && ($_GET['page']=="articles" || $_GET['page']=="highlights" || $_GET['page']=="features" || $_GET['page']=="budgets_analysis" || $_GET['page']=="unionBudget" || $_GET['page']=="stateBudget")) {
					...
					<div class="bordered">
			        	...
			        	?> frameborder='0' allowtransparency='true' scrolling='no' width="100%" <?php  if($file_extn == 'pdf') { echo " height='1130' "; } ?> ></iframe>
			      ...
			      ?>
			      	...
			      	<div class="pull-right text-right b-margin-10">
			      	 	...
			      	 	<ul class="list-inline" >
				           	...
				           	<li><a href="javascript:void(0)" class="ion-printer btn " onclick="printData('iFramePopupFrame')" title="Print this page">Print this page </a></li>
					...
					<div class="bordered">
			        	...
			        	if($file_extn == 'pdf') {  
			        		...
			        		?> src='<?php echo "-?l=".encrypt_url($getBaseUrl.$file_path); ?>' <?php 	

			        	}  

			        	?> frameborder='0' allowtransparency='true' scrolling='no' width="100%" <?php  if($file_extn == 'pdf') { echo " height='1130' "; } ?> ></iframe> -->
			        	<div onload="calcHeight()"  id='iFramePopupFrame' name='iFramePopupFrame' width="100%" <?php  if($file_extn == 'pdf') { echo " height='1130' "; } ?>><?php
			        		if($file_extn == 'pdf') { 
			        ...
			        <div class="pull-right text-right t-margin-10">
			      	 	...
			      	 	<ul class="list-inline" >
				        	...
				        	<li><input type="hidden" value="<?php echo encrypt_url($file_path); ?>" name="file_path" id="file_path" />
