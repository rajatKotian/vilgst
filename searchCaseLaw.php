...
function reload(form,searchcase,keyword)
	{
		self.location='searchCaseLaw?searchcase=' + result + '&keyword='+keyword;
	}
...
function searchtype(val)
	{
		...
		$('.searchType').hide();
		$('#citationExample').hide();
		$('#searchKeyword').val('');
		if(val == "Citation")
		{
		$('#searchTypeLabel').html("Citation")
		//$('#searchKeyword').removeAttr('maxlength');
			$('#citationExample').show();
		}
		else if(val == "PartyName")
		{
		$('#searchTypeLabel').html("PartyName");
		//$('#searchKeyword').attr('maxlength','60');
		}
		else if(val == "Keyword")
		{
...
<div class="col-md-11 col-sm-9 left-section">
	...
	<div class="col-md-16 table-container t-margin-20" >
		<form name="form2" method="get" class="form padding-b-15 "  enctype="multipart/form-data" onSubmit="return ValidateForm()"> 
			...
			<div class="form-group">
				<select name="searchType"  id="searchType" class="form-control" onchange="searchtype(this.value)">
		        	<option value="PartyName" <?php  if(isset($_GET['searchType']) && ($_GET['searchType'] == 'PartyName')){ echo "selected=selected";}?>>Party Name</option>
		            <option value="Citation" <?php  if(isset($_GET['searchType']) && ($_GET['searchType'] == 'Citation')){ echo "selected=selected";}?>>Citation No.</option>
			...
			<div>
			    ...
			    <?php 
				    ...
				    ?>
				    />
					<?php  if(isset($_GET['searchType']) && ($_GET['searchType'] == 'Citation')){ echo "<span style='float:left; margin-top:10px;' id=''>(e.g. 2015-VIL-100-BOM-ST)</span>";} else { ?>
			...
			<div class="form-group">
				...
				<input type="hidden" name="e" value="20" >
		 	...
		 	<div><a href="searchCaseLawAdv" class="pull-right">Advance Search</a></div>
		</form>
	</div> 
	...
	<?php
		...
		AND sp.sub_prod_type ='Judgements' 
		...
		$searchKeyword = searchCombination($searchKeyword);
		...
		$sql =  "SELECT vd.data_id, DATE_FORMAT(circular_date,GET_FORMAT(DATE,'EUR'))'Date', 
			...
			AND vd.sub_prod_id = sp.sub_prod_id
			AND vd.active_flag = 'Y' 
			AND SUBSTR(vd.cir_subject,1,230) REGEXP '".$searchKeyword."'
			AND sp.sub_prod_type ='Judgements' 
			order by vd.data_id DESC ";
			$sqlWithLimit = $sql ." ".$limit;
		}
...
//print_r($searchKeyword);
		...
		if($num_rows == 0) {
			...
			You can also try <a href='searchCaseLawAdv'>Advance Search</a> to explore more.  </div>\n";
		} else {
	 		...
	 		<a href='searchCaseLaw' style='float:right;'>Reset Current Search Result</a></div>\n";
		}		
	?>
	<?php
