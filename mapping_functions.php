...
function customSortSectionRules($x1, $y1) {
    
    $a1 =  $x1['display_name'];
    $b1 =  $y1['display_name'];
    
    $a = trim(str_replace('Section', '', $a1));
    $a = trim(str_replace('Rule', '', $a));
    $b = trim(str_replace('Section', '', $b1));
    $b = trim(str_replace('Rule', '', $b));
    
    if(strpos($a, "(")){
        $start_pos_a = strpos($a, "(");
        $end_pos_a = strpos($a, ")");
        $str_len_a = $end_pos_a - $start_pos_a;
        $p = trim(substr($a, strpos($a, "(")+1, $str_len_a-1));
    }else{
        $p=$a;
    }
    $x = (int)$p;
    ...
    if(strpos($b, "(")){
        ...
        $end_pos_b = strpos($b, ")");
        $str_len_b = $end_pos_b - $start_pos_b;
        $q = trim(substr($b, strpos($b, "(")+1, $str_len_b-1));
    }else{
        $q=$b;
    }
    ...
    $xn = (int) preg_replace('/[^0-9]/', '', $p);
    $yn = (int) preg_replace('/[^0-9]/', '', $q);
    
    if(substr_count(strtolower($a1),'form') > 0){
        return 1;
    }
    
    if(substr_count(strtolower($b1),'form') > 0){
        return -1;
    }
    ...
    if($xn == $yn && ($p!=$q))
    {
            $x_alphabet = str_replace($xn,'',$p);
            $y_alphabet = str_replace($yn,'',$q);
           // echo $x_alphabet .":". $y_alphabet;die;
            
            // if($y_alphabet==){
                
            // }
            
            return ($x_alphabet < $y_alphabet) ? -1:1;
    }
...
function getSimilarCaseLaws($data_id, $data_type) {
    ...
    $headerSectionName = 'TAGGED CASES';
    if ($articleType == 'Judgements' || $articleType == 'Acts') {
        ...
        foreach ($cgstSections as $cgstSec) {
            ...
            $cgstSecionSearchSTR .= " section_no LIKE '%, $cgstSec,%' ";
            $cgstSecionSearchSTR .= " OR section_no = '$cgstSec' ";
...
//                    $commonDataByCase[$r['data_id']] = getCommonRowData($r, $productsList[$r['prod_id']]);
        ...
        foreach ($cgstRules as $cgstRule) {
            ...
            if (strpos($cgstRule, ')')) {
                ...
                $cgstRuleSearchSTR .= " OR rule_no LIKE '$cgstRule(%' OR ";
            }

            $cgstRuleSearchSTR .= " rule_no LIKE '%, $cgstRule,%' ";
            $cgstRuleSearchSTR .= " OR rule_no = '$cgstRule' ";
        ...
        foreach ($igstSections as $igstSec) {
			...
			if ($articleType == 'Acts') {
                ...
                $igstSecionSearchSTR .= " OR igst_section_no LIKE '$igstSec(%'  OR";
            } else {
				if (strpos($igstSec, ')')) {
					$igstSecionSearchSTR .= " igst_section_no LIKE '%, $igstSec(%' ";
					$igstSecionSearchSTR .= " OR igst_section_no LIKE '$igstSec(%' OR ";
            ...
            $igstSecionSearchSTR .= " igst_section_no LIKE '%, $igstSec,%' ";
			$igstSecionSearchSTR .= " OR igst_section_no = '$igstSec' ";
            ...
            while ($r = mysqli_fetch_assoc($result_primary)) {
                ...
                if ($articleType == 'Acts') {
                    ...
                    foreach ($finalSections as $sec) {
                        //$commonData['Section ' . $sec][] = $commonRowData;
                        $commonData['Section ' . $sec][] = getCommonRowData($r, $productsList[$r['prod_id']]);
                    }
                } else {
...
//                $commonDataByCase[$r['data_id']] = getCommonRowData($r, $productsList[$r['prod_id']]);
        ...
        foreach ($igstRules as $igstRule) {
            ...
            if (strpos($igstRule, ')')) {
                ...
                $igstRuleSearchSTR .= " OR igst_rule_no LIKE '$igstRule(%' OR ";
            }

            $igstRuleSearchSTR .= " igst_rule_no LIKE '%, $igstRule,%' ";
            $igstRuleSearchSTR .= " OR igst_rule_no = '$igstRule' ";
...
function getMappingInfoByDataId($data_id, $data_type, $format = 'array') {
    ...
    // print_r($common_data);
    // die;
    $actsData = $common_data['Acts'];
    usort($actsData, "customSortSectionRules");
    $common_data['Acts'] = $actsData;
    
    $rulesData = $common_data['Rules'];
    usort($rulesData, "customSortSectionRules");
    $common_data['Rules'] = $rulesData;
...
function getMappingInfoByDataId2($data_id, $data_type, $format = 'array') {
    ...
    $actsData = $common_data['Acts'];
    if($_GET['filterBy']=='Harshal'){
        print_r($actsData);die;
    }
    uksort($actsData, "customSortSection");
    $common_data['Acts'] = $actsData;
    // uksort($common_data['Rules'], "customSortRule");
