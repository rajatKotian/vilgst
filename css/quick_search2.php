		      	...
		      	function notification(){
					...
					if($_REQUEST['notification_prod_id']!=0)
					{
						...
						if (
							...
							$conditions[] = "(a.circular_date >= " . $yearFrom . " AND a.circular_date <= ". $yearTo .")";
						}
...
<div class="col-md-11 col-sm-9 left-section main_div">
	...
	<div class=" show-more">
		<a href="#." id="advance_search">New Search / Quick Search &nbsp;<i class="ion-chevron-right"></i><i
			...
			<div id="case_law_section">
				<form name="form1" method="get" class="form padding-b-15" id="case_form" action="quick_search.php">
					...
					<div class="form-group">
							...
							<div class="form-group">
							    <select name="case_sub_prod_id" id="sub_prod_id" class="form-control">
							        <option value="0" <?php if(isset($_REQUEST['sub_prod_id']) && ($_REQUEST['sub_prod_id'] == "0")){ echo "selected=selected";}?> data-dbsuffix="0" >Select</option>
							        <?php if (isset($_REQUEST['prod_id'])) { 
							            $statement = mysqli_prepare($GLOBALS['con'], "SELECT sub_prod_name, sub_prod_id FROM sub_product WHERE prod_id = ?");
  
                                        mysqli_stmt_bind_param($statement, 'i', $_REQUEST['prod_id']);
                            	  	...
                            	  	?>
			                		<option value="<?php echo $row['sub_prod_id']; ?>" <?php if(isset($_REQUEST['sub_prod_id']) && ($_REQUEST['sub_prod_id'] == $row['sub_prod_id'])){ echo "selected=selected";}?> data-dbsuffix="0" ><?php echo $row['sub_prod_name']; ?></option>
			                		<?php } } ?>
			                	</select>
							</div>
						</div>
