...
if(isset($_POST['submit']))
{
	...
	// $key = "test_708ea19fd3f9172a8b822329cb1";
	// $token = "test_0a246fae262ea599a34c5339a5d";
	// $mojourl = "test.instamojo.com";

	$key = "431e88618bb01789513ae18aa18d5187";
	$token = "8dd361e5bb99881772dbba50cf0c4282";
	$mojourl = "www.instamojo.com";
	...
	$ch = curl_init();
		...
		$payload = Array(
			...
			'redirect_url' => 'http://www.vilgst.com/payments/Response',
			'send_email' => false,
			'webhook' => '',
			'send_sms' => false,
			'email' => "$email",
		...
		$txnrefno = $decode->payment_request->id;
		$userid = $_SESSION['id'];
...
{	
	...
	return true
 	...
 	<form action="<?php echo $action;?>" method="POST"  name="TransactionForm" id="transactionForm"  onSubmit="return ValidateForm()">
		...
		$orderAmountRowOther = $orderAmountPackage / 100 * $taxOtherVal;
		$orderAmountFinal = number_format ($orderAmountPackage + $orderAmountRowST + $orderAmountRowEC + $orderAmountRowOther,2, '.', '');
	...
	<!-- <input  class="inputTxt" name="returnUrl" type="hidden" value="http://www.vilgst.com/payments/new-Response.php" />
	<input  class="inputTxt" name="notifyUrl" type="hidden" value="http://www.vilgst.com/payments/new-Response.php" /> -->
	...
	<fieldset class="form padding-t-20">
		<h3>Personal Details</h3>
			...
			<label>GSTIN</label>
			<div class="form-group">
				<input class="form-control" type="text" name="txtocc" id="txtocc" placeholder="Subscriber GSTIN" value="<?php echo $row['occ'];?>"/>			
			</div>

			<label>Designation</label>
			<div class="form-group">
				<input class="form-control" type="text" name="txtcodesg" id="txtcodesg" placeholder="Designation" value="<?php echo $row['comapany_desg'];?>"/>
			</div>
...
<!-- COD section END -->
	<div class="col-md-8 t-margin-10 table-container">
		<fieldset class="form padding-t-20">
			...
			<div>If you have opted for updates on additional email-id, please send all such email-ids at <a href="mailto:support@vilgst.com">support@vilgst.com</a></div>
		</fieldset>
	</div>	
<?php } ?>
	</form>
...
<script>
	function addemailCalc(val) {
		...
		var orderAmountRowOther = (eval(orderAmountPackage) + eval(addemailres))  / 100 * eval(taxOther);	
		var orderAmount = eval(orderAmountPackage) + eval(addemailres) + eval(orderAmountRowST) + eval(orderAmountRowEC) + eval(orderAmountRowOther);
