...
<div class="col-md-11 col-sm-9 left-section main_div">
    ...
    <div class="col-md-16">
        ...
        if (isset($_GET['data'])) {
            ...
            if (!isset($_GET['page']) || !is_numeric($page)) {
                $page = 0;
            }
            ...
            if ($_GET['data'] == 'Judgements') {
            ...
            } else if ($_GET['data'] == 'articles' || $_GET['data'] == 'taxvista' || $_GET['data'] == 'highlights' || $_GET['data'] == 'features' || $_GET['data'] == 'budgets_analysis') {

                if ($_GET['data'] == 'taxvista') {
                    $tableName = "taxvista";
                    $additionalFields = "article_id 'id', DATE_FORMAT( article_date, GET_FORMAT( DATE, 'EUR' ) )  'Date'";
                    $whereQuery = "   ";
                } else if ($_GET['data'] == 'articles') {
                    $tableName = "articles";
                ...
                if ($_GET['data'] == 'taxvista' || $_GET['data'] == 'articles' ) {
                    $sql1 = "SELECT  $additionalFields, subject, summary
                          FROM $tableName  
                          $whereQuery
                          ORDER BY article_date DESC";
                }else if ($_GET['data'] == 'features') {
                    $sql1 = "SELECT  $additionalFields, subject, summary
                          FROM $tableName  
                          $whereQuery
                          ORDER BY feature_date DESC";
                } else {
                    $sql1 = "SELECT  $additionalFields, subject, summary, new_flag
                          FROM $tableName  
                          $whereQuery
...
//        echo $sql;die;
                ...
                $lastPage = floor($totalRecords / $limit);
                if ($to > $totalRecords) {
                    $to = $totalRecords;
                }
                $pageBaseUrl = $getBaseUrl . "showMoreData?data=" . $_GET['data'];
                if ($totalRecords == 0) {
                    ...
                    if ($_GET['data'] == 'taxvista' || $_GET['data'] == 'articles') {
                        $sql = "SELECT  $additionalFields, subject, summary
                          FROM $tableName  
                          $whereQuery
                          ORDER BY article_date DESC LIMIT $limit OFFSET $offset";
                    }else if ($_GET['data'] == 'features') {
                        $sql = "SELECT  $additionalFields, subject, summary
                          FROM $tableName  
                          $whereQuery
                          ORDER BY feature_date DESC LIMIT $limit OFFSET $offset";
                    }  else {
                        $sql = "SELECT  $additionalFields, subject, summary, new_flag
                          FROM $tableName  
                          $whereQuery
                          ORDER BY updated_dt DESC LIMIT $limit OFFSET $offset";
                    }
...
//                  echo $sql;die;
                    ...
                    echo "<a ";
                    echo "href='#.' style='color:black;'>Showing $from to $to of <b>$totalRecords Records</b></a>";
                    ...
                    <nav class="navigation pagination pagination1 fontNeuron" role="navigation">
                        <ul class="pagination">
                            ...
                            <li class="page-item active">
                                <a class="page-link" href="<?php echo $pageBaseUrl; ?>&page=<?php echo 0; ?>" tabindex="-1" aria-label="Previous">
                            ...
                            <?php
                            //}
                            if ($current_page >= 2 && $prev_page != 1) {
                                ?>
                                <li class="page-item">
                                    <a class="page-numbers" href="<?php echo $pageBaseUrl; ?>&page=<?php echo $prev_page - 1 ?>"><?php echo $prev_page; ?></a>
                                </li>
                                <?php
                            }
                            $c_page = $current_page;
                            for ($i = 1; $i <= 10; $i++) {
                                ...
                                if ($c_page <= $lastPage) {
                                    if ($c_page == 1) {
                                        ...
                                        ?>">
                                            <a class="page-numbers" href="<?php echo $pageBaseUrl; ?>&page=<?php echo $c_page - 1; ?>"><?php echo $c_page++; ?></a>
                                        </li>
                                        <?php
                                    }
                                }
                            ...
                            ?>
                            <li class="page-item">
                                <a class="page-numbers" href="<?php echo $pageBaseUrl; ?>&page=<?php echo $lastPage ?>" aria-label="Next">
                    ...
                    if ($_GET['data'] == 'taxvista') {
                        echo "<div class='widget-box'>";
                           while ($row = mysqli_fetch_array($result)) {
                            ...
                            echo "<h4><span style='float:left;margin-right:10px'> " . $row['Date'] . " : </span> <a href='javascript:void(0)' onclick='showFrame(\"$encryptID\",\"$tableName\")'> " . $row['subject'] . "</a></h4>";
                        }
                        echo "</div>";
                    } else {
