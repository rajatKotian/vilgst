				...
				<form name="form1" method="get" class="form padding-b-15" id="case_form" action="quick_search.php">
					...
					<div class="form-group" id="noti_year_range" style="display: block;">
						...
						<button type="button" name="resetDates" onclick="resetYearField()" class="btn" style="background: linear-gradient(to bottom, #00789e 0%, #00548a 100%); color: #fff; font-weight: normal; font-size: 12px; text-transform: uppercase; border: none;max-width: 4em;max-height: 35px;">
							<img src="<?php echo $getBaseUrl; ?>images/reset.png" alt="Reset Image" style="max-width: 25px;">
						</button>
				 	</div>
					...
					<!-- <div class="form-group">
						...
						<div class="form-group">
							<select name="court[]" id="forum" data-style="btn-default" class="selectpicker form-control" multiple
								...
								<option value="HC" <?php if (isset($_REQUEST['court']) && in_array("HC", (array)$_REQUEST['court'])) {
									echo "selected=selected" ; } ?> data-dbsuffix="0">High Court</option>
								<option value="TRI" <?php if (isset($_REQUEST['court']) && in_array("TRI", (array)$_REQUEST['court'])) {
									echo "selected=selected" ; } ?> data-dbsuffix="0">CESTAT Cases</option>
								<option value="AAR" <?php if (isset($_REQUEST['court']) && in_array("AAR", (array)$_REQUEST['court'])) {
									echo "selected=selected" ; } ?> data-dbsuffix="0">AAR</option>
								<option value="NAA" <?php if (isset($_REQUEST['court']) && in_array("NAA", (array)$_REQUEST['court'])) {
									echo "selected=selected" ; } ?> data-dbsuffix="0">NAA</option>
								<option value="AAAR" <?php if (isset($_REQUEST['court']) && in_array("AAAR", (array)$_REQUEST['court'])) {
...
<script>
    $(document).ready(function() {
      // Add custom styles when the dropdown-menu is shown
      $('.dropdown').on('show.bs.dropdown', function () {
        $('.dropdown-menu', this).addClass('custom-dropdown-menu');
      });

      // Remove custom styles when the dropdown-menu is hidden
      $('.dropdown').on('hide.bs.dropdown', function () {
        $('.dropdown-menu', this).removeClass('custom-dropdown-menu');
      });
    });
  </script>