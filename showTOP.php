                ...
                <div class="box-body" id="dataresult-container" style="padding-top: 20px;width:80%;margin:auto">
                    <table id="dataResult" class="display table table-bordered table-hover table-blue coi-table" cellspacing="0" width="60%">
                        <tbody>
                            ...
                            $sectionQuery = "SELECT * from coi_sections WHERE section_type='Preamble' AND section_act_type='TRANSFER' order by section_seq_no ASC";
...
<!--<tr><th>asdas</th><th>asdsad</th></tr>-->
                            ...
                            $query = "SELECT * from coi_sections_chapter where section_act_type='TRANSFER' AND chapter_seq_no not in (9998,9999,9997) order by chapter_seq_no ASC";
                        ...
                        <th>" . strtoupper($row['chapter_name']) . " </th>
                                ...
                                $sectionQuery = "SELECT * from coi_sections WHERE chapter_id=" . $row['id'] . " AND section_type='Section' AND section_act_type='TRANSFER' order by section_seq_no ASC";
                                $SectionResult = mysqli_query($GLOBALS['con'],$sectionQuery);
                                while ($sectionRow = mysqli_fetch_assoc($SectionResult)) {
//                                    print_r($articleRow);
                        ...
                        <td>" . strtoupper($sectionRow['section_name']) . " </td>
                            ...
                            $sectionQuery = "SELECT * from coi_sections WHERE section_type='Schedule' AND section_act_type='TRANSFER' order by section_seq_no ASC";
                            $SectionResult = mysqli_query($GLOBALS['con'],$sectionQuery);
                            while ($sectionRow = mysqli_fetch_assoc($SectionResult)) {
//                                    print_r($articleRow);
                                //showing articles data.
                                $encryptyId = base64_encode(base64_encode($sectionRow['id']));
                                $tableData .="<tr>
                        <td><a target='_blank' href='" . $getBaseUrl . "showiframe3?V1Zaa1VsQlJQVDA9=$encryptyId&showLOAsections'>Schedule " . $sectionRow['section_no'] . " </a></td>
                        <td>" . strtoupper($sectionRow['section_name']) . " </td>
                        </tr>";
                            }
                            ...
                            $sectionQuery = "SELECT * from coi_sections WHERE section_type='Appendix' AND section_act_type='TRANSFER' order by section_seq_no ASC";
                            $SectionResult = mysqli_query($GLOBALS['con'],$sectionQuery);
                            while ($sectionRow = mysqli_fetch_assoc($SectionResult)) {
//                                    print_r($articleRow);
                                //showing articles data.
