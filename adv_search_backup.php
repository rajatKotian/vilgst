		...
		if(isset($_REQUEST['searchButton'])) 
		{
			...
			if($_REQUEST['pagename']=='Notification')
		    {
				...
				$dbsuffix=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['dbsuffix']);
		      	...
		      	function notification(){
			     	$prod_id=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['prod_id']);//product_id
					$state_id=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['state_id']);//state_id
					if(isset($_REQUEST['type']))
					{
						$not_type=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['type']);//notification type
					}

					if(isset($_REQUEST['sub_product_id']))
					{
						$sub_product_id=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['sub_product_id']);//sub product id
					}
					$text=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['text']);//text
					$not_no=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['no']);//notification number
					$date=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['date']);//exact date
					$fromDate=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['fromDate']);//from date
					$toDate=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['toDate']);// to date
					$dbsuffix=mysqli_real_escape_string($GLOBALS['con'],$_REQUEST['dbsuffix']);// dbsuffix
				    ...
				    if(isset($_REQUEST['type']) && $not_type!='0') {
				    	if($not_type=="circulars")
				    	{
				    		$conditions[] = "(a.sub_subprod_id = '$not_type' OR a.sub_subprod_id='')";
				    	}
				    	else
				    	{
				    ...
				    if(isset($_REQUEST['sub_product_id']) && $sub_product_id!='0') {
				      $conditions[] = "(a.sub_prod_id = '$sub_product_id')";
				    }
				    if(! empty($date)) {
				    ...
				    if($dbsuffix=="0"){
				    	...
				    	$conditions[]="(".getsubproductnoti('sub_product','Notifications',$table['id']).")";
				    	//return $conditions;
				    	//$table=array("casedata_ce","casedata_cu","casedata_gst","casedata_st","casedata_vat");
						$query=allCaseDataQuery($conditions,$table['db']);
					}else if(isset($_REQUEST['sub_product_id']) && $sub_product_id=='0'){
						$conditions[]="(".getsubproductnoti('sub_product','Notifications',$prod_id).")";
			...
			if($_REQUEST['pagename']=='Case Laws'){
				...
				function case_data(){
				    ...
				    if($dbsuffix=="0"){
				    	$table=getCategory('category','Judgements');
...
$(document).ready(function(){
	...
	var query1="searchbody.php?search="+value+"&"+query;
	//alert(query1);
	$.ajax({
        //data :{search: value},
        ...
        success: function(html){
            	...
            	if(html!=""){
            		...
            		if(value == "Notification"){
            			...
            			var cat_type=$("#cat_type").val(); //sub product _id like notification or circular
            			var prodid=$("#prod_id").val();
            			if(prodid!="" || prodid!='0'){
            				//alert(cat_type+','+not_type);
							...
							$.ajax({
				            	data :{id: prodid, table : table, sub_pid : cat_type},
				            	...
				            	success: function(data){
					            	...
					            	if(prodid=='7' || prodid=='8' || prodid=='9' || prodid=='10')
									{
					...
					$("#prod_id").change(function(){
						...
						if(val=='4' || val=='5' || val=='7' || val=='8' || val=='9' || val=='10')
						{
							$.ajax({
